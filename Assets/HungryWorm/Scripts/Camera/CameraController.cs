...
namespace HungryWorm
{
    ...
    {
        private Transform m_TargetedObject;
        [SerializeField] private float m_SmoothTime = 0.1f;

        [Header("Margins")] [SerializeField] private float depth = -10;
        [SerializeField] private float m_HorizontalMargin = 0.4f;
        [SerializeField] private float m_VerticalMargin = 0.45f;
        [SerializeField] private float m_AirTopMargin = 0.3f;
        [SerializeField] private float m_GroundedBottomMargin = 0.2f;
        [SerializeField] private float m_DeepGroundedBottomMargin = 0.5f;


        [Header("World Slices settings")] [SerializeField]
        private float m_bufferSize = 5f;
        ...
        private void SetTarget()
        {
            ...
            {
                Debug.LogError("Targeted object of CameraController is null");
            }
            ...
            Debug.DrawLine(m_Camera.ScreenToWorldPoint(new Vector3(bottomLeft.x, bottomLeft.y, 10)),
                m_Camera.ScreenToWorldPoint(new Vector3(topRight.x, bottomLeft.y, 10)), Color.red);
            Debug.DrawLine(m_Camera.ScreenToWorldPoint(new Vector3(topRight.x, bottomLeft.y, 10)),
                m_Camera.ScreenToWorldPoint(new Vector3(topRight.x, topRight.y, 10)), Color.red);
            Debug.DrawLine(m_Camera.ScreenToWorldPoint(new Vector3(topRight.x, topRight.y, 10)),
                m_Camera.ScreenToWorldPoint(new Vector3(bottomLeft.x, topRight.y, 10)), Color.red);
            Debug.DrawLine(m_Camera.ScreenToWorldPoint(new Vector3(bottomLeft.x, topRight.y, 10)),
                m_Camera.ScreenToWorldPoint(new Vector3(bottomLeft.x, bottomLeft.y, 10)), Color.red);
        ...
        void MoveCamera()
        {
            ...
            m_Camera.transform.position =
                Vector3.SmoothDamp(m_Camera.transform.position, target, ref currentVelocity, m_SmoothTime);
